...
from .base import LoadedMarketData, MarketAdapter, NoImbalanceSettlement, SettlementRule, SinglePriceImbalanceSettlement


class BelgiumMarketAdapter(MarketAdapter):
    market_id = "belgium"
    timezone = "Europe/Brussels"
    resolution_minutes = 15
    supported_workflows = ("da_only", "da_plus_imbalance", "da_plus_fcr", "da_plus_afrr")
    day_ahead_zone = "10YBE----------2"
    imbalance_zone = "10YBE----------2"
    fcr_zone = "10YBE----------2"
    afrr_zone = "10YBE----------2"
    ...
    def validate_timing(self, config: BacktestConfig) -> None:
        if config.timing.timezone != self.timezone:
            raise ValueError(f"Belgium adapter requires timezone={self.timezone}")
        if config.timing.resolution_minutes != self.resolution_minutes:
            raise ValueError("Belgium adapter only supports 15-minute resolution")

    def settlement_engine(self, workflow: str) -> SettlementRule:
        if workflow == "da_only":
            return NoImbalanceSettlement()
        if workflow == "da_plus_imbalance":
            return SinglePriceImbalanceSettlement()
        if workflow in {"da_plus_fcr", "da_plus_afrr"}:
            return NoImbalanceSettlement()
        raise ValueError(f"Unsupported Belgium workflow: {workflow}")
